...
from models.core.ecc import ECC
# import sys
# sys.path.append('thirdparty/box-mot')
...
class TrackManager:
    ...
    @torch.no_grad()
    def update(self,cur_frame:int,current_detections:np.ndarray,img_date:np.ndarray) -> List[Tracker]:
        '''
        current_detections =np.ndarray(tlwh,conf)  and have already filtered by conf > 0.1 
        '''
        warp_mtx = self.cmc.apply(img_date,None)

        img_rgb = cv2.cvtColor(img_date,cv2.COLOR_BGR2RGB)
        img_tensor = torch.from_numpy(img_rgb).permute(2,0,1).to(self.device).to(torch.float32)
        ...
        if len(first_tras_list ) > 0:
            for track in first_tras_list :
                track.camera_update(warp_mtx)
        ...
        det_graph  = self.construct_det_graph(first_dets_list,img_tensor)
        ...
        if second_tras_list :
            ...
            # if []:
                ...
                #---------------------------------#
                second_det_graph = self.construct_det_graph(second_dets_list,img_tensor)
