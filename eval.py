...
from models.graphTracker_kf import TrackManager
    
def parse_args():
    parser = argparse.ArgumentParser(description='Track evaluation')
    parser.add_argument('--dataset', type=str, default='MOT17', help='dataset name')
    parser.add_argument('--tracker', type=str, default='DGCTracker', help='TrackerName for TrackEval Evalution')
...
def main():
    ...
    for cnt, seq in enumerate(seq_name_list):
        ...
        for frame_id in tqdm(range(min_frame,max_frame+1),total=max_frame,desc=f'processing seq - {seq} ',unit='frame'):
            ...
            img_bgr    = img_data.clone().permute(1,2,0).numpy()[...,::-1].astype(np.uint8)
            # img_data = cv2.imread(os.path.join(seq_img_dir,f'{frame_id:06d}.jpg'))
            # if frame_id == 23:
            #     print('here')
            ...
            online_im = plot_tracking(
                img_bgr,online_tlwhs, online_ids, frame_id=frame_id, fps=int(info_dict['frameRate'])
            )
